...
'''
def bb84_xorpuf4(puf_bit, puf_basis, position, k, steps, success_prob):
	...
	if position == 'bit':
		accuracy_cpuf = instance_one_apuf_attack_n(puf_bit, crps, repeat_experiment, steps)
		accuracy_hpuf = instance_one_hybrid_apuf_attack_n(success_prob, puf_bit, puf_basis, crps, position, repeat_experiment, steps)
		# np.save('./data/xorpuf4/'+str(n)+'n_xorpuf4_crps.npy', crps)
		# np.save('./data/xorpuf4/'+str(n)+'c_xorpuf4_a.npy', accuracy_cpuf)
		# np.save('./data/xorpuf4/'+str(n)+'h_xorpuf4_'+position+'_a.npy', accuracy_hpuf)
	elif position == 'basis':
...
if __name__ == '__main__':
	'''
	Simulation of HLPUF with BB84 encoding and an underlying of 4XORPUF against adpative adversaries
	
	Variables:
	n: length of challenge, it defaults to 64 bits
	noisiness_cpuf: noisiness, it effects the reliability(robustness) of CPUF instance
	...
	crps_bit, accuracy_cpuf_bit, accuracy_hpuf_bit = bb84_xorpuf4(puf_bit, puf_basis, 'bit', k, steps, success_prob)
	_, _, accuracy_hpuf_basis = bb84_xorpuf4(puf_bit, puf_basis, 'basis', k, steps, success_prob)
