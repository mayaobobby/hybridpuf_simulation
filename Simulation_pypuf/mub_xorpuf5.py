...
if __name__ == '__main__':
    ...
    for i in range(3):
        ...
        while(accuracy < 0.95):    
            ...
            num_crps += step 
            np.save('./mub_data/hybrid_xorpuf_accuracy_bit_'+str(i)+'_n'+str(n_size)+'k'+str(k)+'_MUB8_rep'+str(repeat_experiment)+'.npy', accuracies)
            if i == 0:
                np.save('./mub_data/classical_xorpuf_accuracy_n'+str(n_size)+'k'+str(k)+'_MUB8_rep'+str(repeat_experiment)+'.npy', accuracies_c)
    np.save('./mub_data/crps_xorpuf_n'+str(n_size)+'k'+str(k)+'_MUB8_rep'+str(repeat_experiment)+'.npy', num_samples)