...
crps = np.load('./crps_xorpuf_n'+str(n_size)+'k'+str(k)+'_MUB4_rep'+str(repeat_experiment)+'.npy')
accuracy_c = np.load('./classical_xorpuf_accuracy_n'+str(n_size)+'k'+str(k)+'_MUB4_rep'+str(repeat_experiment)+'.npy')
...
colors = ['green','blue']
for i in range(2):
    accuracy_h = np.load('./hybrid_xorpuf_accuracy_bit_'+str(i)+'_n'+str(n_size)+'k'+str(k)+'_MUB4_rep'+str(repeat_experiment)+'.npy')
    if(i > 0):
...
with open('./d4n32k5.txt', 'a') as f:
		...
		accuracy_h_0 = np.load('./hybrid_xorpuf_accuracy_bit_0_n'+str(n_size)+'k'+str(k)+'_MUB4_rep'+str(repeat_experiment)+'.npy')
		accuracy_h_1 = np.load('./hybrid_xorpuf_accuracy_bit_1_n'+str(n_size)+'k'+str(k)+'_MUB4_rep'+str(repeat_experiment)+'.npy')
		
		mu, sigma = 0.5, 0.01 # mean and standard deviation
		s = list(np.random.normal(mu, sigma, len(accuracy_h_0)))
		hlpuf_accuracy_final = s+accuracy_h_1.flatten().tolist()
